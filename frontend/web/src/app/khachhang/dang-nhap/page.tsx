...
import { gfetch, setToken } from "@/lib/auth-guest"; // 👈 dùng wrapper & lưu guest_token
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const data = await gfetch("/api/auth/guest/login", {
                method: "POST",
                body: JSON.stringify({ KH_TAIKHOAN: username, KH_MATKHAU: password }),
            });
            // BE trả { token, guest }
            setToken(data.token); // lưu "guest_token"
            const redirect =
                ...
                <div>
                    ...
                    <form onSubmit={onSubmit}>
                        <div className="space-y-6">
                            <div>
                                <Label>Tên tài khoản <span className="text-error-500">*</span></Label>
                                <Input
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="block font-normal text-gray-700 text-theme-sm dark:text-gray-400">Ghi nhớ tôi</span>
                                </div>
                    ...
                    <div className="mt-5">
                        <p className="text-sm font-normal text-center text-gray-700 dark:text-gray-400 sm:text-start">
                            Không có tài khoản?{" "}
                            <Link href="/khachhang/dang-ky" className="text-brand-500 hover:text-brand-600 dark:text-brand-400">
                                Đăng ký ngay!
                            </Link>
                        </p>
                    </div>
